...
import type { Cell, Defaults, RenderOptions } from "./types.ts";
...
export function ansi256ToHex(code: number): string | null {
  ...
  if (code <= 231) {
    ...
    return rgbToHex(levels[r] ?? 0, levels[g] ?? 0, levels[b] ?? 0);
  }
...
function applyColor(state: Cell, color: string, target: 38 | 48): void {
  if (target === 38) state.fg = color;
  else state.bg = color;
}

function applySgrCodes(codes: number[], state: Cell, defaults: Defaults): Cell {
...
function readEscapeSequence(
  input: string,
  start: number,
): { command: string; values: number[]; endIndex: number } {
  let cursor = start + 2;
  let params = "";
...
export function parseAnsi(input: string, defaults: Defaults): Cell[][] {
  const lines: Cell[][] = [[]];
  let row = 0;
  let col = 0;
  let state: Cell = { char: " ", fg: defaults.fg, bg: null, bold: false };
...
function estimateCellWidth(fontSize: number): number {
...
export function render(lines: Cell[][], options: RenderOptions): Buffer {
